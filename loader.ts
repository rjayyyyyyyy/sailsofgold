...
import Dispatcher, {
  ...
  NetworkEvent,
  SystemEvent,
} from "./src/framework/events/Dispatcher";
import type { ILauncherPayload } from "./src/framework/interfaces/ILauncherPayload";
import {
  type ILauncherConfig,
  sampleConfig,
} from "./src/framework/interfaces/ILauncherConfig";
import {
  ClientCommand,
  Command,
  ServerCommand,
} from "./src/framework/networking/Commands";
...
const game = import.meta.env.VITE_GAME || "sailsofgold";
...
const scriptFile =
  env === "dev"
    ? `./src/games/${gameType}/${game}/main.ts`
    : `./dist/games/${game}/${game}.bundle.js`;
...
async function createSession(token: string) {
  try {
    const response = await fetch(
      "https://api.lydrst.com/api/portal/Users/Sessions",
      {
...
function getDebugSessionID(gameId: string, gameName: string) {
  return new Promise<string>(async (resolve) => {
    ...
    const listener = dispatcher.addListener(
      CommandEvent.GAME_IN,
      (command: Command) => {
        switch (command.type) {
          case ServerCommand.NewSessionId:
            // this.logger.info("New Session ID received");
            if (!hasSession) {
              // no session found previously. Proceed to login
              hasSession = true;
  ...
  .then(async (module) => {
    ...
    const config: ILauncherConfig = {
      ...
      pId: queryParams.get("pid")
        ? parseInt(queryParams.get("pid") as string)
        : 0,
